...
elseif do_explosion == false then --effect should run every 2 seconds but the script runs every 1 second, so i have a script 
...
ComponentObjectSetValue2(comp, "config_explosion", "explosion_radius", 16 * math.min(40, multiplier^.8)) --radius of the explosion (obviously limited for good reasons)
...
ComponentObjectSetValue2(comp, "config_explosion", "sparks_count_min", (6 * math.min(20, multiplier))^2) --minimum particle sparks
ComponentObjectSetValue2(comp, "config_explosion", "sparks_count_max", (13 * math.min(20, multiplier))^2) --maximum particle sparks
...
if GameHasFlagRun("PERK_PICKED_IRON_STOMACH") and b ~= 0 then --if player has picked IRON_STOMACH and Ingestion is not 0:
    ComponentObjectSetValue2(comp, "config_explosion", "dont_damage_this", owner) --set the explosion to not harm the target
end
