...
if count >= 3 then
	if not vomit then
		EntityAddChild( owner, EntityLoad("mods/Hydroxide/files/entities/effects/mutagens/mutagen_vomit.xml", x,y ))
	end
end

if count >= 4 then
	if not leggy then
		EntityAddChild( owner, EntityLoad("mods/Hydroxide/files/entities/effects/mutagens/mutagen_leggy.xml", x, y ))
	end
end

if count >= 6 then
	if not shaking then
		EntityAddChild( owner, EntityLoad("mods/Hydroxide/files/entities/effects/mutagens/mutagen_shaking.xml", x, y ))
	end
end

if count >= 7 then
	if not aiming then
		EntityAddChild( owner, EntityLoad("mods/Hydroxide/files/entities/effects/mutagens/mutagen_aiming.xml", x, y ))
	end
end
