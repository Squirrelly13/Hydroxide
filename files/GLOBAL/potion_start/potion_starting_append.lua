...
starterpotions = CombineTables(starterpotions, modded_starterpotions) --format for combining tables should be lib_table = combine_tabless(lib_table, modded_table)
...
magicpotions = CombineTables(magicpotions, modded_magicpotions)
...
one_in_millions = CombineTables(one_in_millions, modded_one_in_millions)
...
failpotions = CombineTables(failpotions, modded_failpotions)
...
local modded_functions = {
	function(outcome) --if christmas, 2024, then replace water outcome with santanium
		if CompareTables({UTC.year, UTC.month, UTC.day}, {2024, 12, 25}) and outcome == "water" then return "santanium" end
	end,
	...
	function(outcome, r_value)
		if CompareTables({UTC.month, UTC.day}, {4, 1}) then
...
functions = CombineTables(functions, modded_functions)
...
functions2 = CombineTables(functions2, modded_functions2)
